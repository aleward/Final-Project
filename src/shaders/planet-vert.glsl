...
// Planet specific deformation of noise
vec4 planetWarp(vec4 v, vec4 displacement) {
    float n = iterations(v.xyz);
    //n = 1.f - abs(n) * 1.5;
    float water = (sin(u_Time * 0.01) + 1.f) * 0.5 / 15.f;
    if (n < water) n = water;

    return v + (displacement * n * 0.4);
}
...
void main()
{
    ...
    fs_Pos = vs_Pos;
    vec4 newPos = planetWarp(vs_Pos, vs_Nor); // Deformed vertices
    ...
    vec4 compVec1 = planetWarp(compPos1, compPos1) - newPos; 
    vec4 compVec2 = planetWarp(compPos2, compPos2) - newPos;
