...
void main()
{
    ...
    else { specularIntensity = max(pow(dot(normalize(avg), normalize(fs_Nor)), 4.f), 0.f); }

    float val = (snoise(fs_Pos.xyz) + 1.f) * 0.3 - (snoise(fs_Pos.xyz * 6.f) + 1.f) * 0.2;
    //val = sqrt(val);
    ...
    out_Col = vec4(lightIntensity * vec3(245.f / 255.f, 239.f / 255.f, 255.f / 255.f), val);
}
